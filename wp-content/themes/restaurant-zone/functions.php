...
if ( ! function_exists( 'restaurant_zone_setup' ) ) :
	...
	function restaurant_zone_setup() {
		...
		*/
		...
		add_action('wp_ajax_restaurant_zone_dismissable_notice', 'restaurant_zone_dismissable_notice');
...
function restaurant_zone_theme_color() {
    ...
    $restaurant_zone_logo_max_height = get_theme_mod('restaurant_zone_logo_max_height');
    	...
    	if(get_theme_mod('restaurant_zone_preloader_bg_color') == '') {
			$restaurant_zone_preloader_bg_color = '#000000';
		}
		if(get_theme_mod('restaurant_zone_preloader_dot_1_color') == '') {
			$restaurant_zone_preloader_dot_1_color = '#ffffff';
		}
		if(get_theme_mod('restaurant_zone_preloader_dot_2_color') == '') {
			$restaurant_zone_preloader_dot_2_color = '#f10026';
		}
...
function restaurant_zone_sanitize_number_range( $number, $setting ) {
...
function restaurant_zone_getpage_css($hook) {
    ...
    wp_localize_script('admin-notice-script','restaurant_zone',
		array('admin_ajax'	=>	admin_url('admin-ajax.php'),'wpnonce'  =>	wp_create_nonce('restaurant_zone_dismissed_notice_nonce')
		)
	);
	wp_enqueue_script('admin-notice-script');

    wp_localize_script( 'admin-notice-script', 'restaurant_zone_ajax_object',
...
define('RESTAURANT_ZONE_LIVE_DEMO',__('https://demo.themagnifico.net/restaurant-zone/','restaurant-zone'));
}
if ( ! defined( 'RESTAURANT_ZONE_GET_PREMIUM_PRO' ) ) {
define('RESTAURANT_ZONE_GET_PREMIUM_PRO',__('https://www.themagnifico.net/products/restaurant-wordpress-theme','restaurant-zone'));
}
if ( ! defined( 'RESTAURANT_ZONE_PRO_DOC' ) ) {
define('RESTAURANT_ZONE_PRO_DOC',__('https://demo.themagnifico.net/eard/wathiqa/restaurant-pro-doc/','restaurant-zone'));
}
if ( ! defined( 'RESTAURANT_ZONE_FREE_DOC' ) ) {
define('RESTAURANT_ZONE_FREE_DOC',__('https://demo.themagnifico.net/eard/wathiqa/restaurant-zone-free-doc/','restaurant-zone'));
}

add_action('admin_menu', 'restaurant_zone_themepage');
function restaurant_zone_themepage(){
	
	$restaurant_zone_theme_test = wp_get_theme();
...
function restaurant_zone_info_page() {
	...
	<div class="wrap about-wrap restaurant-zone-add-css">
			...
			<div class="feature-section three-col">
				...
				<div class="col">
					<div class="widgets-holder-wrap">
						...
						<p><a target="_blank" href="<?php echo esc_url( RESTAURANT_ZONE_GET_PREMIUM_PRO ); ?>" class="button button-primary get prem">
				...
				<div class="col">
					<div class="widgets-holder-wrap">
						<h3><?php esc_html_e("Free Documentation", "restaurant-zone"); ?></h3>
						<p><?php esc_html_e("Our guide is available if you require any help configuring and setting up the theme. Easy and quick way to setup the theme.", "restaurant-zone"); ?></p>
						<p><a target="_blank" href="<?php echo esc_url( RESTAURANT_ZONE_FREE_DOC ); ?>" class="button button-primary get">
							<?php esc_html_e("Free Documentation", "restaurant-zone"); ?>
						</a></p>
					</div>
				</div>
			</div>
		...
		<div class="restaurant-zone-button-container">
			<a target="_blank" href="<?php echo esc_url( RESTAURANT_ZONE_GET_PREMIUM_PRO ); ?>" class="button button-primary get prem">
...
function restaurant_zone_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
function restaurant_zone_deprecated_hook_admin_notice() {

    $dismissed = get_user_meta(get_current_user_id(), 'restaurant_zone_dismissable_notice', true);
    if ( !$dismissed) { ?>
        <div class="updated notice notice-success is-dismissible notice-get-started-class" data-notice="get_started" style="background: #f7f9f9; padding: 20px 10px; display: flex;">
	    	...
	    	<div class="tm-admin-content" style="padding-left: 30px; align-self: center">
	    		<h2 style="font-weight: 600;line-height: 1.3; margin: 0px;"><?php esc_html_e('Thank You For Choosing ', 'restaurant-zone'); ?><?php echo wp_get_theme(); ?><h2>
	    		<p style="color: #3c434a; font-weight: 400; margin-bottom: 30px;"><?php _e('Get Started With Theme By Clicking On Getting Started.', 'restaurant-zone'); ?><p>
	        	<a class="admin-notice-btn button button-primary button-hero" href="<?php echo esc_url( admin_url( 'themes.php?page=restaurant-zone-info.php' )); ?>"><?php esc_html_e( 'Get started', 'restaurant-zone' ) ?></a>
	        	<a class="admin-notice-btn button button-primary button-hero" target="_blank" href="<?php echo esc_url( RESTAURANT_ZONE_FREE_DOC ); ?>"><?php esc_html_e( 'Documentation', 'restaurant-zone' ) ?></a>
	        	<span style="padding-top: 15px; display: inline-block; padding-left: 8px;">
	        	<span class="dashicons dashicons-admin-links"></span>
	        	<a class="admin-notice-btn"	 target="_blank" href="<?php echo esc_url( RESTAURANT_ZONE_LIVE_DEMO ); ?>"><?php esc_html_e( 'View Demo', 'restaurant-zone' ) ?></a>
	        	</span>
	    	</div>
        </div>
    <?php }
...
add_action( 'admin_notices', 'restaurant_zone_deprecated_hook_admin_notice' );

function restaurant_zone_switch_theme() {
    delete_user_meta(get_current_user_id(), 'restaurant_zone_dismissable_notice');
}
add_action('after_switch_theme', 'restaurant_zone_switch_theme');
function restaurant_zone_dismissable_notice() {
    update_user_meta(get_current_user_id(), 'restaurant_zone_dismissable_notice', true);
    die();
}